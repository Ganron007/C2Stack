...
# Public IP passed via Terraform template variable (Azure IMDS doesn't expose public IP)
PUBLIC_IP="$GUAC_PUBLIC_IP"
...
if [ -n "$TOKEN" ]; then
    ...
    # Create SSH connection to Guacamole Server (use template variable for private IP)
    GUAC_PRIVATE_IP="$GUACAMOLE_PRIVATE_IP"
    curl -s -X POST "http://localhost:8080/guacamole/api/session/data/postgresql/connections?token=$TOKEN" \
        -H "Content-Type: application/json" \
        -d "{
            \"name\": \"Guacamole (SSH)\",
